 ...
 * https://m-katsurada.sakura.ne.jp/program/fdm/heat2d-i.c
 * https://m-katsurada.sakura.ne.jp/program/fdm/smallmatrix.h
 * https://m-katsurada.sakura.ne.jp/program/fdm/symbandlu.c
 * https://m-katsurada.sakura.ne.jp/program/fdm/symbandlu.h
 *
 * To get these files
 ...
 * or
 *     ccmg heat2d-i.c symbandlu.c -DOLD
 *
 */
...
int main()
{
    ...
    if ((B = malloc(sizeof(double) * NN)) == NULL) {
        fprintf(stderr, "B を記憶する領域の確保に失敗\n");
        exit(1);
    }
    if ((vector_U = malloc(sizeof(double) * NN)) == NULL) {
        fprintf(stderr, "vector_U を記憶する領域の確保に失敗\n");
        exit(1);
    }
    if ((iwork = malloc(sizeof(int) * NN)) == NULL) {
